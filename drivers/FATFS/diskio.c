...
)
{
  ...
  switch (drv)
  {
    ...
    {
	  Status =  microsd_readSingleBlock( sector ,buff );
    }
    else                    /* å¤šä¸ªsectorçš„è¯»æ“�ä½œ */
    {
      Status = microsd_readMultiBlock( sector , buff ,count);
    }
    if(Status == 0)
    {
      return RES_OK;
...
)
{
  ...
  switch (drv)
  {
    ...
    {
       Status = microsd_writeSingleBlock( sector , (uint8_t *)(&buff[0]) );
    }
    else                    /* å¤šä¸ªsectorçš„å†™æ“�ä½œ */
    {
       Status = microsd_writeMultiBlock( sector , (uint8_t *)(&buff[0]) , count );
    }
    if(Status == 0)
    {
       return RES_OK;
