...
static uint8_t wizchip_rw(uint8_t byte){
	...
	while (SPI_I2S_GetFlagStatus(W5500_SPI_PORT, SPI_I2S_FLAG_TXE) == RESET);

	/*!< Send byte through the SPI2 peripheral */
	SPI_I2S_SendData(W5500_SPI_PORT, byte);

	/*!< Wait to receive a byte */
	while (SPI_I2S_GetFlagStatus(W5500_SPI_PORT, SPI_I2S_FLAG_RXNE) == RESET);

	/*!< Return the byte read from the SPI bus */
	return SPI_I2S_ReceiveData(W5500_SPI_PORT);
}


static void  wizchip_select(void)
{
	GPIO_ResetBits(W5500_SPI_CS_GPIO_PORT, W5500_SPI_CS_PIN);
}

static void  wizchip_deselect(void)
{
	GPIO_SetBits(W5500_SPI_CS_GPIO_PORT, W5500_SPI_CS_PIN);
}
...
void w5500_init(void){
	...
	/*!< W5500 CS/MOSI/MISO/CLK Pin GPIO Peripheral clock enable */
	RCC_APB1PeriphClockCmd(	W5500_SPI_SCK_GPIO_CLK |
							W5500_SPI_MISO_GPIO_CLK |
							W5500_SPI_MOSI_GPIO_CLK |
							W5500_SPI_CS_GPIO_CLK |
							W5500_SPI_RST_GPIO_CLK |
							W5500_SPI_INT_GPIO_CLK, ENABLE);

	/*!< W5500_SPI Peripheral clock enable */
	RCC_APB2PeriphClockCmd(W5500_SPI_CLOCK, ENABLE);
	...
	GPIO_Init(W5500_SPI_CS_GPIO_PORT, &GPIO_InitStructure);

	/*!< Configure W5500_RST_PIN pin: W5500 RST pin */
	GPIO_InitStructure.GPIO_Pin = W5500_SPI_RST_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(W5500_SPI_RST_GPIO_PORT, &GPIO_InitStructure);

    GPIO_SetBits(W5500_SPI_RST_GPIO_PORT, W5500_SPI_RST_PIN);

    GPIO_InitStructure.GPIO_Pin = W5500_SPI_INT_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
    GPIO_Init(W5500_SPI_INT_GPIO_PORT, &GPIO_InitStructure);
	...
	SPI_Init(W5500_SPI_PORT, &SPI_InitStructure);

	/*!< Enable the W5500_SPI  */
	SPI_Cmd(W5500_SPI_PORT, ENABLE);

	GPIO_ResetBits(GPIOB, W5500_SPI_RST_PIN);
	...
	/*!< W5500_SPI Peripheral clock enable */
	RCC_APB1PeriphClockCmd(W5500_SPI_CLOCK, ENABLE);
